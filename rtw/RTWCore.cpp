...
OSDefineMetaClassAndStructors(RTWCore, IOEthernetController)
OSDefineMetaClassAndStructors(CTimeout, OSObject)

IOWorkLoop *_fWorkloop;
IOCommandGate *_fCommandGate;

bool RTWCore::init(OSDictionary *properties)
{
...
IOService* RTWCore::probe(IOService *provider, SInt32 *score)
{
...
bool RTWCore::configureInterface(IONetworkInterface *netif)
{
    IONetworkData *nd;
...
struct _ifnet *RTWCore::getIfp()
{
    return &fHalService->get80211Controller()->ic_ac.ac_if;
}

IOEthernetInterface *RTWCore::getNetworkInterface()
{
    return getIfp()->iface;
}

bool RTWCore::createMediumTables(const IONetworkMedium **primary)
{
...
void RTWCore::joinSSID(const char *ssid_name, const char *ssid_pwd)
{
...
void RTWCore::associateSSID(const char *ssid, const char *pwd)
{
    struct ieee80211com *ic = fHalService->get80211Controller();
    if (strlen(pwd) == 0) {
...
bool RTWCore::start(IOService *provider)
{
    int boot_value = 0;
    ...
    _fCommandGate = IOCommandGate::commandGate(this, (IOCommandGate::Action)RTWCore::tsleepHandler);
    if (_fCommandGate == 0) {
    ...
    watchdogTimer = IOTimerEventSource::timerEventSource(this, OSMemberFunctionCast(IOTimerEventSource::Action, this, &RTWCore::watchdogAction));
    if (!watchdogTimer) {
...
void RTWCore::watchdogAction(IOTimerEventSource *timer)
{
...
const OSString * RTWCore::newVendorString() const
{
    return OSString::withCString("Apple");
}

const OSString * RTWCore::newModelString() const
{
    return OSString::withCString("Intel Wireless Card");
}

bool RTWCore::initPCIPowerManagment(IOPCIDevice *provider)
{
    UInt16 reg16;
...
bool RTWCore::createWorkLoop()
{
    _fWorkloop = IOWorkLoop::workLoop();
    return _fWorkloop != 0;
}

IOWorkLoop *RTWCore::getWorkLoop() const
{
    return _fWorkloop;
}

IOReturn RTWCore::selectMedium(const IONetworkMedium *medium) {
    setSelectedMedium(medium);
    return kIOReturnSuccess;
}

void RTWCore::stop(IOService *provider)
{
    XYLog("%s\n", __FUNCTION__);
    struct _ifnet *ifp = &fHalService->get80211Controller()->ic_ac.ac_if;
    super::stop(provider);
...
IOReturn RTWCore::enable(IONetworkInterface *netif)
{
    XYLog("%s\n", __FUNCTION__);
    super::enable(netif);
...
IOReturn RTWCore::disable(IONetworkInterface *netif)
{
    XYLog("%s\n", __FUNCTION__);
    super::disable(netif);
    watchdogTimer->cancelTimeout();
...
IOReturn RTWCore::getHardwareAddress(IOEthernetAddress *addrP)
{
...
IOReturn RTWCore::setHardwareAddress(const IOEthernetAddress *addrP)
{
    if (!fNetIf || !addrP)
        return kIOReturnError;
...
#ifdef __PRIVATE_SPI__
IOReturn RTWCore::outputStart(IONetworkInterface *interface, IOOptionBits options)
{
    struct _ifnet *ifp = &fHalService->get80211Controller()->ic_ac.ac_if;
    mbuf_t m = NULL;
...
UInt32 RTWCore::outputPacket(mbuf_t m, void *param)
{
//    XYLog("%s\n", __FUNCTION__);
...
UInt32 RTWCore::getFeatures() const
{
    return fHalService->getDriverInfo()->supportedFeatures();
}

IOReturn RTWCore::setPromiscuousMode(IOEnetPromiscuousMode mode)
{
    return kIOReturnSuccess;
}

IOReturn RTWCore::setMulticastMode(IOEnetMulticastMode mode)
{
    return kIOReturnSuccess;
}

IOReturn RTWCore::setMulticastList(IOEthernetAddress* addr, UInt32 len)
{
    return fHalService->getDriverController()->setMulticastList(addr, len);
}

IOReturn RTWCore::getPacketFilters(const OSSymbol *group, UInt32 *filters) const
{
...
{
    RTWCore* dev = OSDynamicCast(RTWCore, owner);
    if (dev == 0)
        return kIOReturnError;
