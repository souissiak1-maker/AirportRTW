...
#ifdef RTW89_MACOS
extern void rtw89_macos_set_null_ab_global_u32(unsigned int group,
                                               unsigned int index,
                                               u32 value);
#endif
...
hdr:
#ifdef RTW89_MACOS
	/* 0.2.33 CAM/CCTL station-data-context trace.  Dump the exact payload
	 * words submitted to firmware after all version-specific fields are set. */
	{
		const __le32 *dw = (const __le32 *)h2c_v0;
		struct rtw89_addr_cam_entry *ac =
			rtw89_get_addr_cam_of(rtwvif_link, rtwsta_link);
		struct rtw89_bssid_cam_entry *bc =
			rtw89_get_bssid_cam_of(rtwvif_link, rtwsta_link);
		const u8 *vmac = rtwvif_link->mac_addr;
		const u8 *bssid = rtwvif_link->bssid;
		u8 vm = rtwvif_link->mac_id;
		u8 sm = rtwsta_link ? rtwsta_link->mac_id : vm;
		u32 i;

		rtw89_info(rtwdev,
		    "[RTW89-CONTEXT][CAM] upd=%u ver=%u len=%u "
		    "vif_macid=%u sta_macid=%u port=%u net=%u aid=%u "
		    "addr_valid=%u addr_idx=%u addr_bssid_idx=%u "
		    "bssid_valid=%u bssid_idx=%u phy=%u "
		    "vifmac=%02x:%02x:%02x:%02x:%02x:%02x "
		    "bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
		    upd_mode, ver, len, vm, sm, rtwvif_link->port,
		    rtwvif_link->net_type,
		    rtwsta_link ? rtwsta_link_to_sta(rtwsta_link)->aid : 0,
		    ac ? ac->valid : 0, ac ? ac->addr_cam_idx : 0,
		    ac ? ac->bssid_cam_idx : 0,
		    bc ? bc->valid : 0, bc ? bc->bssid_cam_idx : 0,
		    bc ? bc->phy_idx : 0,
		    vmac[0], vmac[1], vmac[2], vmac[3], vmac[4], vmac[5],
		    bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5]);
		rtw89_macos_set_null_ab_global_u32(1, 1, upd_mode);
		rtw89_macos_set_null_ab_global_u32(1, 2, ver);
		rtw89_macos_set_null_ab_global_u32(1, 3, len);
		rtw89_macos_set_null_ab_global_u32(1, 4, vm);
		rtw89_macos_set_null_ab_global_u32(1, 5, sm);
		rtw89_macos_set_null_ab_global_u32(1, 6, rtwvif_link->port);
		rtw89_macos_set_null_ab_global_u32(1, 7, rtwvif_link->net_type);
		rtw89_macos_set_null_ab_global_u32(1, 8,
			rtwsta_link ? rtwsta_link_to_sta(rtwsta_link)->aid : 0);
		rtw89_macos_set_null_ab_global_u32(1, 9, ac ? ac->valid : 0);
		rtw89_macos_set_null_ab_global_u32(1, 10, ac ? ac->addr_cam_idx : 0);
		rtw89_macos_set_null_ab_global_u32(1, 11, ac ? ac->bssid_cam_idx : 0);
		rtw89_macos_set_null_ab_global_u32(1, 12, bc ? bc->valid : 0);
		rtw89_macos_set_null_ab_global_u32(1, 13, bc ? bc->bssid_cam_idx : 0);
		rtw89_macos_set_null_ab_global_u32(1, 14, bc ? bc->phy_idx : 0);
		for (i = 0; i < len / sizeof(*dw); i++)
			rtw89_macos_set_null_ab_global_u32(2, i,
							le32_to_cpu(dw[i]));

		for (i = 0; i < len / sizeof(*dw); i += 4) {
			u32 n = min_t(u32, 4, len / sizeof(*dw) - i);
			rtw89_info(rtwdev,
			    "[RTW89-CONTEXT][CAM-DW] first=%u n=%u "
			    "%08x %08x %08x %08x\n",
			    i, n,
			    n > 0 ? le32_to_cpu(dw[i]) : 0,
			    n > 1 ? le32_to_cpu(dw[i + 1]) : 0,
			    n > 2 ? le32_to_cpu(dw[i + 2]) : 0,
			    n > 3 ? le32_to_cpu(dw[i + 3]) : 0);
		}
	}
#endif
...
#ifdef RTW89_MACOS
	rtw89_info(rtwdev,
	    "[RTW89-CONTEXT][CAM-RESULT] upd=%u ret=%d\n",
	    upd_mode, ret);
	rtw89_macos_set_null_ab_global_u32(1, 15, (u32)ret);
#endif
	if (ret) {
		rtw89_err(rtwdev, "failed to send h2c\n");
		goto fail;
...
#ifdef RTW89_MACOS
	{
		const __le32 *dw = (const __le32 *)skb->data;
		u32 i;
		rtw89_info(rtwdev,
		    "[RTW89-CONTEXT][CCTL] len=%u macid=%u sta=%u "
		    "port=%u net=%u band=%u lowest=0x%04x func=%u\n",
		    H2C_CMC_TBL_LEN, mac_id, rtwsta_link ? 1 : 0,
		    rtwvif_link->port, rtwvif_link->net_type,
		    chan->band_type, lowest_rate, chip->h2c_cctl_func_id);
		rtw89_macos_set_null_ab_global_u32(3, 1, H2C_CMC_TBL_LEN);
		rtw89_macos_set_null_ab_global_u32(3, 2, mac_id);
		rtw89_macos_set_null_ab_global_u32(3, 3, rtwsta_link ? 1 : 0);
		rtw89_macos_set_null_ab_global_u32(3, 4, rtwvif_link->port);
		rtw89_macos_set_null_ab_global_u32(3, 5, rtwvif_link->net_type);
		rtw89_macos_set_null_ab_global_u32(3, 6, chan->band_type);
		rtw89_macos_set_null_ab_global_u32(3, 7, lowest_rate);
		rtw89_macos_set_null_ab_global_u32(3, 8, chip->h2c_cctl_func_id);
		for (i = 0; i < H2C_CMC_TBL_LEN / sizeof(*dw); i++)
			rtw89_macos_set_null_ab_global_u32(4, i,
							le32_to_cpu(dw[i]));

		for (i = 0; i < H2C_CMC_TBL_LEN / sizeof(*dw); i += 4) {
			u32 n = min_t(u32, 4,
				H2C_CMC_TBL_LEN / sizeof(*dw) - i);
			rtw89_info(rtwdev,
			    "[RTW89-CONTEXT][CCTL-DW] first=%u n=%u "
			    "%08x %08x %08x %08x\n",
			    i, n,
			    n > 0 ? le32_to_cpu(dw[i]) : 0,
			    n > 1 ? le32_to_cpu(dw[i + 1]) : 0,
...
#ifdef RTW89_MACOS
	rtw89_info(rtwdev,
	    "[RTW89-CONTEXT][CCTL-RESULT] macid=%u ret=%d\n",
	    mac_id, ret);
	rtw89_macos_set_null_ab_global_u32(3, 9, (u32)ret);
#endif
	if (ret) {
		rtw89_err(rtwdev, "failed to send h2c\n");
		goto fail;
...
extern void rtw89_macos_ra_audit_u32(unsigned int field, unsigned int value);
extern void rtw89_macos_ra_audit_u64(unsigned int field, unsigned long long value);
#endif
...
#ifdef RTW89_MACOS
	/* Stage C remains the pre-submit health check from 0.2.41. */
	rtw89_macos_assoc_stage_probe(3);
	rtw89_macos_ra_audit_u32(32, len);
	rtw89_macos_ra_audit_u32(33, ver);
	rtw89_macos_ra_audit_u32(34, le32_to_cpu(h2c->w0));
	rtw89_macos_ra_audit_u32(35, le32_to_cpu(h2c->w1));
	rtw89_macos_ra_audit_u32(36, le32_to_cpu(h2c->w2));
	rtw89_macos_ra_audit_u32(37, le32_to_cpu(h2c->w3));
	rtw89_macos_ra_audit_u32(38, ver >= 1 ? le32_to_cpu(h2c_v1->w4) : 0);
	rtw89_macos_ra_audit_u32(39, chip->chip_gen);
	rtw89_macos_ra_audit_u32(40, 1);
#endif
...
#ifdef RTW89_MACOS
	rtw89_macos_assoc_stage_probe(4);
#endif
