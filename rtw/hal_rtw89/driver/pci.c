...
extern void rtw89_macos_set_null_ab_variant_u32(unsigned int variant,
                                                unsigned int field,
                                                u32 value);
static const char *rtw89_macos_pci_diag_tag(struct sk_buff *skb);
				       ...
				       struct rtw89_pci_rx_ring *rx_ring)
{
	...
	if (ls) {
		struct sk_buff *complete = new;

		/* Transfer ownership before entering the core RX/report path.  The
		 * PPDU-status path frees @complete synchronously.  Leaving the same
		 * pointer published in diliver_skb until after the callback lets any
		 * accidental/re-entrant poll observe and free it a second time. */
		rx_ring->diliver_skb = NULL;
		desc_info->ready = false;
		rtw89_core_rx(rtwdev, desc_info, complete);
	}
...
#ifdef RTW89_MACOS
	/* 0.2.34 NULL A/B raw completion bridge plus the 0.2.32 log. */
	{
		const char *diag_tag = rtw89_macos_pci_diag_tag(skb);
		if (skb->pkt_type >= 0xB1 && skb->pkt_type <= 0xB4) {
			u32 v = skb->pkt_type - 0xB0;
			rtw89_macos_set_null_ab_variant_u32(v, 34, tx_status);
			rtw89_macos_set_null_ab_variant_u32(v, 35,
							tx_status == RTW89_TX_DONE);
			rtw89_macos_set_null_ab_variant_u32(v, 36,
							tx_status == RTW89_TX_RETRY_LIMIT);
			rtw89_macos_set_null_ab_variant_u32(v, 37,
							tx_status == RTW89_TX_LIFE_TIME);
			rtw89_macos_set_null_ab_variant_u32(v, 38,
							tx_status == RTW89_TX_MACID_DROP);
			rtw89_macos_set_null_ab_variant_u32(v, 39, tx_ring->tx_cnt);
			rtw89_macos_set_null_ab_variant_u32(v, 40, tx_ring->tx_acked);
		}
		if (diag_tag)
			rtw89_info(rtwdev,
			    "[RTW89-DIAG][%s][TXRPT] raw=0x%02x done=%u "
			    "retry_limit=%u lifetime=%u macid_drop=%u "
			    "ring_tx=%llu ring_ack=%llu\n",
			    diag_tag, tx_status, tx_status == RTW89_TX_DONE,
			    tx_status == RTW89_TX_RETRY_LIMIT,
			    tx_status == RTW89_TX_LIFE_TIME,
			    tx_status == RTW89_TX_MACID_DROP,
			    (unsigned long long)tx_ring->tx_cnt,
			    (unsigned long long)tx_ring->tx_acked);
	}
#endif
...
static void rtw89_macos_diag_serialized_txwd(struct rtw89_dev *rtwdev,
					     struct rtw89_core_tx_request *tx_req,
					     const void *txdesc, u32 txwd_len,
					     u32 txwp_len, u32 txaddr_len)
{
	const struct rtw89_tx_desc_info *d = &tx_req->desc_info;
	const __le32 *dw = txdesc;
	const char *tag;
	u32 words;
	u32 i;

	tag = rtw89_macos_pci_diag_tag(tx_req->skb);
	if (!tag)
		return;

	words = min_t(u32, txwd_len / sizeof(*dw), 16);
	rtw89_info(rtwdev,
		    "[RTW89-DIAG][%s][TXWD] wdlen=%u wplen=%u addrlen=%u "
		    "total=%u addrinfo=%u macid=%u qsel=%u chdma=%u port=%u\n",
		    tag, txwd_len, txwp_len, txaddr_len,
		    txwd_len + txwp_len + txaddr_len, d->addr_info_nr,
		    d->mac_id, d->qsel, d->ch_dma, d->port);

	if (tx_req->skb->pkt_type >= 0xB1 && tx_req->skb->pkt_type <= 0xB4) {
		u32 v = tx_req->skb->pkt_type - 0xB0;
		rtw89_macos_set_null_ab_variant_u32(v, 31, txwd_len);
		rtw89_macos_set_null_ab_variant_u32(v, 32, txwp_len);
		rtw89_macos_set_null_ab_variant_u32(v, 33, txaddr_len);
		for (i = 0; i < words; i++)
			rtw89_macos_set_null_ab_variant_u32(v, 41 + i,
							le32_to_cpu(dw[i]));
	}

	for (i = 0; i < words; i += 4) {
		u32 v0 = le32_to_cpu(dw[i]);
		u32 v1 = i + 1 < words ? le32_to_cpu(dw[i + 1]) : 0;
		u32 v2 = i + 2 < words ? le32_to_cpu(dw[i + 2]) : 0;
		u32 v3 = i + 3 < words ? le32_to_cpu(dw[i + 3]) : 0;
		rtw89_info(rtwdev,
			    "[RTW89-DIAG][%s][TXWD-DW] first=%u n=%u "
			    "%08x %08x %08x %08x\n",
			    tag, i, min_t(u32, 4, words - i),
			    v0, v1, v2, v3);
	}
}
#endif /* RTW89_MACOS */
...
#ifdef RTW89_MACOS
	rtw89_macos_diag_serialized_txwd(rtwdev, tx_req, txwd->vaddr,
					 txwd_len, txwp_len, txaddr_info_len);
#endif
...
#ifdef RTW89_MACOS
extern void rtw89_macos_set_probe_stage(const char *stage, int code);
#define RTW89_MACOS_PROBE_STAGE(stage, code) \
	rtw89_macos_set_probe_stage((stage), (code))
#else
#define RTW89_MACOS_PROBE_STAGE(stage, code) do { } while (0)
#endif
...
int rtw89_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	RTW89_MACOS_PROBE_STAGE("probe-alloc-hw-running", 0);
	rtwdev = rtw89_alloc_ieee80211_hw(&pdev->dev,
					  sizeof(struct rtw89_pci),
					  info->chip, info->variant);
	if (!rtwdev) {
		dev_err(&pdev->dev, "failed to allocate hw\n");
		RTW89_MACOS_PROBE_STAGE("probe-alloc-hw-failed", -ENOMEM);
		return -ENOMEM;
	}
	RTW89_MACOS_PROBE_STAGE("probe-alloc-hw-complete", 0);
	...
	RTW89_MACOS_PROBE_STAGE("probe-core-init-running", 0);
	ret = rtw89_core_init(rtwdev);
	if (ret) {
		rtw89_err(rtwdev, "failed to initialise core\n");
		RTW89_MACOS_PROBE_STAGE("probe-core-init-failed", ret);
		goto err_release_hw;
	}
	RTW89_MACOS_PROBE_STAGE("probe-core-init-complete", 0);

	RTW89_MACOS_PROBE_STAGE("probe-pci-claim-running", 0);
	ret = rtw89_pci_claim_device(rtwdev, pdev);
	if (ret) {
		rtw89_err(rtwdev, "failed to claim pci device\n");
		RTW89_MACOS_PROBE_STAGE("probe-pci-claim-failed", ret);
		goto err_core_deinit;
	}
	RTW89_MACOS_PROBE_STAGE("probe-pci-claim-complete", 0);

	RTW89_MACOS_PROBE_STAGE("probe-pci-resource-running", 0);
	ret = rtw89_pci_setup_resource(rtwdev, pdev);
	if (ret) {
		rtw89_err(rtwdev, "failed to setup pci resource\n");
		RTW89_MACOS_PROBE_STAGE("probe-pci-resource-failed", ret);
		goto err_declaim_pci;
	}
	RTW89_MACOS_PROBE_STAGE("probe-pci-resource-complete", 0);

	RTW89_MACOS_PROBE_STAGE("probe-chip-info-running", 0);
	ret = rtw89_chip_info_setup(rtwdev);
	if (ret) {
		rtw89_err(rtwdev, "failed to setup chip information\n");
		RTW89_MACOS_PROBE_STAGE("probe-chip-info-failed", ret);
		goto err_clear_resource;
	}
	RTW89_MACOS_PROBE_STAGE("probe-chip-info-complete", 0);

	rtw89_pci_basic_cfg(rtwdev, false);

	RTW89_MACOS_PROBE_STAGE("probe-napi-init-running", 0);
	ret = rtw89_core_napi_init(rtwdev);
	if (ret) {
		rtw89_err(rtwdev, "failed to init napi\n");
		RTW89_MACOS_PROBE_STAGE("probe-napi-init-failed", ret);
		goto err_clear_resource;
	}
	RTW89_MACOS_PROBE_STAGE("probe-napi-init-complete", 0);

	RTW89_MACOS_PROBE_STAGE("probe-request-irq-running", 0);
	ret = rtw89_pci_request_irq(rtwdev, pdev);
	if (ret) {
		rtw89_err(rtwdev, "failed to request pci irq\n");
		RTW89_MACOS_PROBE_STAGE("probe-request-irq-failed", ret);
		goto err_deinit_napi;
	}
	RTW89_MACOS_PROBE_STAGE("probe-request-irq-complete", 0);

	RTW89_MACOS_PROBE_STAGE("probe-core-register-running", 0);
	ret = rtw89_core_register(rtwdev);
	if (ret) {
		rtw89_err(rtwdev, "failed to register core\n");
		RTW89_MACOS_PROBE_STAGE("probe-core-register-failed", ret);
		goto err_free_irq;
	}
	RTW89_MACOS_PROBE_STAGE("probe-core-register-complete", 0);

	set_bit(RTW89_FLAG_PROBE_DONE, rtwdev->flags);
	RTW89_MACOS_PROBE_STAGE("probe-complete", 0);
