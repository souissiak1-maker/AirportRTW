			...
			u8 wl_state, u8 bt_state, u8 wlact_state)
{
	...
	if (!btc->ver->fcxosi) {
		/*
		 * Route coexistence grant programming through the active chip ops.
		 * RTL8852A uses rtw89_mac_cfg_gnt(), while newer BE chips select
		 * rtw89_mac_cfg_gnt_v2/v3 from their own chip tables.  Calling v2
		 * directly here leaves an unresolved BE-only symbol in an 8852A-only
		 * kext and OpenCore rejects the prelinked injection.
		 */
		rtw89_chip_mac_cfg_gnt(rtwdev, &dm->gnt);
		return;
	}
			  ...
			  u32 len, u8 class_id, u8 func)
{
	...
	rtw89_debug(rtwdev, RTW89_DBG_BTC,
		    ...
		    __func__, len, class_id, func);

	if (class_id != BTFC_FW_EVENT)
		return;
