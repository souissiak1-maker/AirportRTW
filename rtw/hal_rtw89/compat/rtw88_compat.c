...
/* fls: declared extern in the MacKernelSDK libkern.h but not KPI-exported */
...
static void wq_thread_fn(void *arg, wait_result_t wr)
{
    ...
    wq->done = 1;
	IOLockWakeup(wq->lock, (void *)(uintptr_t)&wq->done, false);
    IOLockUnlock(wq->lock);
...
void destroy_workqueue(struct workqueue_struct *wq)
{
    ...
    while (!wq->done)
		IOLockSleep(wq->lock, (void *)(uintptr_t)&wq->done, THREAD_UNINT);
    IOLockUnlock(wq->lock);
    IOLockFree(wq->lock);
    IOFree(wq, sizeof(*wq));
}
...
{
    (void)flags;
    /* Linux timers remain reusable after del_timer_sync().  Keep the XNU
     * thread_call allocated as well: freeing and reallocating it here races
     * a concurrent cancel path and can leave an invalid thread-call index. */
    if (timer->call)
...
static void rtw88_napi_thread_wrapper(thread_call_param_t param0, thread_call_param_t param1)
{
    ...
    if (!napi || !napi->poll) {
        if (napi)
            __sync_lock_test_and_set(&napi->running, 0);
        return;
    }

    /* Consume the request represented by this invocation.  Any interrupt or
     * power-path schedule that arrives while poll() is running sets pending=1
     * instead of entering the same thread_call again.  The old shim never used
     * napi->running, so two callbacks could own the same RX ring/diliver_skb at
     * once; a PPDU report could then be passed to rtw89_core_rx() twice and the
     * second path hit kfree_skb() on an already-freed object. */
    __sync_lock_test_and_set(&napi->pending, 0);

    /* Drain in a bounded loop.  rtw89_pci_napi_poll() re-arms the interrupt
     * only when work < weight; when the budget is exhausted Linux reschedules
     * NAPI.  Keep the existing bounded-drain behavior, but serialize ownership. */
    do {
        work = napi->poll(napi, napi->weight);
    } while (work >= napi->weight && ++iters < 64);

    if (work >= napi->weight)
        __sync_lock_test_and_set(&napi->pending, 1);

    /* Publish completion before checking for a coalesced request.  This closes
     * both races: a scheduler that observed running=1 leaves pending set for us;
     * a scheduler that wins running 0->1 after this point queues the next call
     * itself, and our compare-and-swap below simply fails. */
    __sync_synchronize();
    __sync_lock_test_and_set(&napi->running, 0);
    __sync_synchronize();

    if (__sync_fetch_and_add(&napi->pending, 0) &&
        __sync_bool_compare_and_swap(&napi->running, 0, 1) &&
        napi->thread_call)
...
void rtw88_napi_schedule(struct napi_struct *napi)
{
    ...
    /* Coalesce all producers into a single NAPI owner.  The owner checks
     * pending again after dropping running, so no interrupt is lost in the
     * hand-off window. */
    __sync_lock_test_and_set(&napi->pending, 1);
    __sync_synchronize();
    if (__sync_bool_compare_and_swap(&napi->running, 0, 1))
        thread_call_enter((thread_call_t)napi->thread_call);
}
                                        ...
                                        int link_id, bool qos_ok)
{
	...
	u8 *p = (u8 *)skb_put_zero(skb, hdr_len);
                                      ...
                                      struct ieee80211_vif *vif)
{
	...
	u8 *p = (u8 *)skb_put_zero(skb, frame_len);
 ...
 * bridge (rtw89's Senmiko.c) instead. */
...
{
    (void)tid;
    return rtw88_get_tx_ring_snapshot(rtwdev, snapshot);
}
...
/* The legacy rtw88 target already programs its receive configuration through
 * the older RCR path.  Keep a no-op implementation so the shared MLME links;
 * AirportRTW89 supplies the rtw89-specific implementation in Senmiko.c. */
