//
//  AirportRTWSkywalkInterface.cpp
//  AirportRTW-Sonoma
//
...
OSDefineMetaClassAndStructors(AirportRTWSkywalkInterface, IO80211InfraProtocol);
...
void AirportRTWSkywalkInterface::associateSSID(uint8_t *ssid, uint32_t ssid_len, const struct ether_addr &bssid, uint32_t authtype_lower, uint32_t authtype_upper, uint8_t *key, uint32_t key_len, int key_index)
{
...
void AirportRTWSkywalkInterface::setPTK(const u_int8_t *key, size_t key_len) {
    struct ieee80211com *ic = fHalService->get80211Controller();
...
void AirportRTWSkywalkInterface::setGTK(const u_int8_t *gtk, size_t key_len, u_int8_t kid, u_int8_t *rsc) {
    struct ieee80211com *ic = fHalService->get80211Controller();
    struct ieee80211_node    * ni = ic->ic_bss;
    struct ieee80211_key *k;
    int keylen;
...
bool AirportRTWSkywalkInterface::
init(IOService *provider)
{
    ...
    instance = OSDynamicCast(AirportRTW, provider);
    if (!instance)
        return false;
...
//ifnet_t AirportRTWSkywalkInterface::
//getBSDInterface()
//{
...
IOReturn AirportRTWSkywalkInterface::
