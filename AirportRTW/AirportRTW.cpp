...
IOService* AirportRTW::probe(IOService *provider, SInt32 *score)
{
    ...
    if (!device)
        return NULL;
    IOLog("AirportRTW: probe PCI %04x:%04x\n",
    ...
    if (ItlRtw89::rtw89_match(device)) {
        ...
        IOLog("AirportRTW-RTW89: probe accepted (MSI=%u MSI-X=%u); "
              "transport will configure interrupts\n", msiCap, msixCap);
        return this;
    }
    return NULL;
}
...
{
    AirportRTWInterface *netif = new AirportRTWInterface;
    if (!netif)
        return NULL;
...
void AirportRTW::associateSSID(uint8_t *ssid, uint32_t ssid_len, const struct ether_addr &bssid, uint32_t authtype_lower, uint32_t authtype_upper, uint8_t *key, uint32_t key_len, int key_index) 
{
...
void AirportRTW::setPTK(const u_int8_t *key, size_t key_len) {
    struct ieee80211com *ic = fHalService->get80211Controller();
    ...
    if (ni->ni_rsncipher != IEEE80211_CIPHER_USEGROUP) {
        ...
        /* Apple's external supplicant can complete the four-way handshake
         * without issuing a separate GTK callback.  Once the PTK is safely
         * installed, open the controlled port and protect both directions;
         * otherwise ieee80211_encap() drops every non-EAPOL packet and DHCP
         * never reaches the hardware.  A later GTK callback still installs
         * the group key normally. */
...
void AirportRTW::setGTK(const u_int8_t *gtk, size_t key_len, u_int8_t kid, u_int8_t *rsc) {
    struct ieee80211com *ic = fHalService->get80211Controller();
    struct ieee80211_node	* ni = ic->ic_bss;
    struct ieee80211_key *k;
    int keylen;
...
bool AirportRTW::createWorkLoop()
{
    _fWorkloop = IO80211WorkLoop::workLoop();
    return _fWorkloop != 0;
}

IOWorkLoop *AirportRTW::getWorkLoop() const
{
    return _fWorkloop;
}
...
IOReturn AirportRTW::enable(IONetworkInterface *netif)
{
    XYLog("%s\n", __PRETTY_FUNCTION__);
    super::enable(netif);
...
IOReturn AirportRTW::disable(IONetworkInterface *netif)
{
    XYLog("%s\n", __PRETTY_FUNCTION__);
    super::disable(netif);
    setLinkStatus(kIONetworkLinkValid);
...
IOReturn AirportRTW::setHardwareAddress(const IOEthernetAddress *addrP)
{
    if (!fNetIf || !addrP)
        return kIOReturnError;
...
IOReturn AirportRTW::getHardwareAddressForInterface(
...
SInt32 AirportRTW::monitorModeSetEnabled(
...
IO80211VirtualInterface *AirportRTW::
