...
SInt32 AirportRTW::apple80211Request(unsigned int request_type,
                                       int request_number,
                        ...
                        struct apple80211_rssi_data *rd)
{
    ...
    if (ic->ic_state == IEEE80211_S_RUN) {
        ...
        = -(0 - RTW89_MIN_DBM - ic->ic_bss->ni_rssi);
        return kIOReturnSuccess;
    }
    return 6;
}
...
IOReturn AirportRTW::setDISASSOCIATE(OSObject *object)
{
    XYLog("%s\n", __FUNCTION__);
    struct ieee80211com *ic = fHalService->get80211Controller();
...
{
    struct ieee80211com *ic = fHalService->get80211Controller();
    const UInt32 scanDoneDelayMS = 100;
    if (OSDynamicCast(ItlRtw89, fHalService)) {
        setProperty("RTW89AppleScanRequestCount", ++rtwScanRequestCount, 64);
        setProperty("RTW89AppleScanRequestState", (UInt64)ic->ic_state, 8);
    }
#if 0
    ...
    if (fScanResultWrapping) {
        if (OSDynamicCast(ItlRtw89, fHalService))
            setProperty("RTW89AppleScanRequestResult", (UInt64)22, 32);
        return 22;
    }
    ...
    if (OSDynamicCast(ItlRtw89, fHalService))
        setProperty("RTW89AppleScanRequestResult", (UInt64)0, 32);
    if (sd->scan_type == APPLE80211_SCAN_TYPE_FAST || sd->scan_type == APPLE80211_SCAN_TYPE_PASSIVE) {
        if (scanSource) {
            scanSource->setTimeoutMS(scanDoneDelayMS);
            scanSource->enable();
        }
        return kIOReturnSuccess;
    }
...
setSCAN_REQ_MULTIPLE(OSObject *object, struct apple80211_scan_multiple_data *sd)
{
    ...
    const UInt32 scanDoneDelayMS = 100;
    if (OSDynamicCast(ItlRtw89, fHalService)) {
        setProperty("RTW89AppleScanMultipleRequestCount",
                    ++rtwScanRequestCount, 64);
        setProperty("RTW89AppleScanRequestState", (UInt64)ic->ic_state, 8);
    }
#if 0
    int i;
...
{
    struct ieee80211com *ic = fHalService->get80211Controller();
    const bool isRtw89 = OSDynamicCast(ItlRtw89, fHalService) != nullptr;
    if (isRtw89) {
        setProperty("RTW89AppleScanResultRequestCount",
                    ++rtwScanResultRequestCount, 64);
        setProperty("RTW89AppleScanResultNodeCount", (UInt64)ic->ic_nnodes, 32);
    }
...
//    XYLog("%s ni_bssid=%s ni_essid=%s channel=%d flags=%d asr_cap=%d asr_nrates=%d asr_ssid_len=%d asr_ie_len=%d asr_rssi=%d\n", __FUNCTION__, ether_sprintf(fNextNodeToSend->ni_bssid), fNextNodeToSend->ni_essid, ieee80211_chan2ieee(ic, fNextNodeToSend->ni_chan), ieeeChanFlag2apple(fNextNodeToSend->ni_chan->ic_flags), fNextNodeToSend->ni_capinfo, fNextNodeToSend->ni_rates.rs_nrates, fNextNodeToSend->ni_esslen, fNextNodeToSend->ni_rsnie_tlv == NULL ? 0 : fNextNodeToSend->ni_rsnie_tlv_len, fNextNodeToSend->ni_rssi);
    ...
    const uint8_t *scanIE = fNextNodeToSend->ni_rsnie_tlv;
    size_t scanIELength = fNextNodeToSend->ni_rsnie_tlv_len;
    /* Keep the complete, bounded beacon/probe IE stream. This matches the
     * upstream AirportRTW Ventura path and preserves SSID, rates, channel,
     * RSN/WPA, HT/VHT/HE and vendor elements for CoreWLAN. */
    if (isRtw89 && scanIE && scanIELength > 0) {
        size_t validLength = 0;
        while (validLength + 2 <= scanIELength) {
            const size_t elementLength = (size_t)scanIE[validLength + 1] + 2;
            if (elementLength > scanIELength - validLength)
                break;
            validLength += elementLength;
        }
        scanIELength = validLength;
    }
    if (scanIE && scanIELength > 0) {
#if __IO80211_TARGET < __MAC_12_0
        result->asr_ie_len = scanIELength;
        result->asr_ie_data = (void *)scanIE;
#else
        result->asr_ie_len = MIN(scanIELength, sizeof(result->asr_ie_data));
        memcpy(result->asr_ie_data, scanIE, result->asr_ie_len);
#endif
    } else {
        result->asr_ie_len = 0;
#if __IO80211_TARGET < __MAC_12_0
    ...
    if (isRtw89) {
        result->asr_nrates = MIN(fNextNodeToSend->ni_rates.rs_nrates,
                                 APPLE80211_MAX_RATES);
        for (uint8_t i = 0; i < result->asr_nrates; i++)
            result->asr_rates[i] = fNextNodeToSend->ni_rates.rs_rates[i];
        /* These nodes belong to the live scan that is being returned now.
         * Avoid having CoreWLAN age out early-channel results while the
         * software sweep spends 5.2 seconds visiting all channels. */
        result->asr_age = 0;
    } else {
        ...
        result->asr_age =
            (uint32_t)(airport_up_time() - fNextNodeToSend->ni_age_ts);
    }
    ...
    result->asr_rssi = -(0 - RTW89_MIN_DBM - fNextNodeToSend->ni_rssi);
    ...
    if (isRtw89) {
        setProperty("RTW89AppleScanResultSSIDLength",
                    (UInt64)result->asr_ssid_len, 8);
        setProperty("RTW89AppleScanResultChannel",
                    (UInt64)result->asr_channel.channel, 16);
        setProperty("RTW89AppleScanResultRSSI",
                    (UInt64)(SInt64)result->asr_rssi, 64);
        setProperty("RTW89AppleScanResultNoise",
                    (UInt64)(SInt64)result->asr_noise, 64);
        setProperty("RTW89AppleScanResultAge",
                    (UInt64)result->asr_age, 32);
        setProperty("RTW89AppleScanResultChannelFlags",
                    (UInt64)result->asr_channel.flags, 32);
        setProperty("RTW89AppleScanResultIELength",
                    (UInt64)(UInt16)result->asr_ie_len, 16);
        setProperty("RTW89AppleScanResultIEType",
                    (UInt64)(result->asr_ie_len > 0
                                 ? reinterpret_cast<const UInt8 *>(
                                       result->asr_ie_data)[0]
                                 : 0), 8);
        setProperty("RTW89AppleScanResultCapability",
                    (UInt64)(UInt16)result->asr_cap, 16);
        setProperty("RTW89AppleScanResultBeaconInterval",
                    (UInt64)(UInt16)result->asr_beacon_int, 16);
        setProperty("RTW89AppleScanResultRateCount",
                    (UInt64)result->asr_nrates, 8);
        setProperty("RTW89AppleScanResultRate0",
...
getLINK_CHANGED_EVENT_DATA(OSObject *object, struct apple80211_link_changed_event_data *ed)
{
    ...
    if (ed->isLinkDown) {
        ...
        ed->rssi = -(0 - RTW89_MIN_DBM - ic->ic_bss->ni_rssi);
