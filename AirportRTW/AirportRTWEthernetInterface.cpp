//
//  AirportRTWEthernetInterface.cpp
//  AirportRTW-Sonoma
//
...
#include "AirportRTWEthernetInterface.hpp"

#include <sys/_if_ether.h>
#include <net80211/ieee80211_var.h>

#define super IOEthernetInterface
OSDefineMetaClassAndStructors(AirportRTWEthernetInterface, IOEthernetInterface);

bool AirportRTWEthernetInterface::
...
IOReturn AirportRTWEthernetInterface::
...
//        ret = bpf_attach(getIfnet(), DLT_RAW, 0x48, &AirportRTWEthernetInterface::bpfOutputPacket, &AirportRTWEthernetInterface::bpfTap);
    }
    isAttach = true;
    return ret;
}

void AirportRTWEthernetInterface::
 ...
 */
IOService *AirportRTWEthernetInterface::
getProvider() const
{
    return isAttach ? this->interface : super::getProvider();
}

errno_t AirportRTWEthernetInterface::
bpfOutputPacket(ifnet_t interface, u_int32_t data_link_type, mbuf_t packet)
{
    ...
    AirportRTWEthernetInterface *networkInterface = (AirportRTWEthernetInterface *)ifnet_softc(interface);
...
UInt32 AirportRTWEthernetInterface::
