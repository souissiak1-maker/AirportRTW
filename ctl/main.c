...
#define RTW88_PRIVATE_CONTROL_USERCLIENT_TYPE 0x52545743U /* 'RTWC' */
#include <mach/mach.h>
...
_Static_assert(sizeof(struct RTW88SyntheticAssociateArgs) == 40,
...
static io_connect_t open_kext(void)
{
     ...
     * differs. Try both. */
    static const char *classes[] = { "AirportRTW89", "RTW89PCIDevice", "RTW88PCIDevice" };
    ...
    kern_return_t kr = IOServiceOpen(service, mach_task_self(),
                                     RTW88_PRIVATE_CONTROL_USERCLIENT_TYPE,
                                     &conn);
    if (kr != KERN_SUCCESS) {
        /* Compatibility fallback for non-Airport/older builds whose private
         * control client still lived at type 0. */
...
static int parse_bssid(const char *text, uint8_t out[6])
{
    unsigned int b[6];
    char tail = '\0';
    if (!text || sscanf(text, "%x:%x:%x:%x:%x:%x%c",
        ...
        return -1;
    for (int i = 0; i < 6; ++i) {
        if (b[i] > 0xffU) return -1;
        out[i] = (uint8_t)b[i];
    }
    return 0;
}

static int cmd_synthetic_associate(io_connect_t conn, const char *ssid,
                                   const char *bssid_text)
{
    struct RTW88SyntheticAssociateArgs args;
    memset(&args, 0, sizeof(args));
    if (!ssid || strlen(ssid) == 0 || strlen(ssid) > 32) {
        fprintf(stderr, "rtw88ctl: synthetic-assoc requires a 1..32-byte SSID\n");
        return 1;
    }
    strlcpy(args.ssid, ssid, sizeof(args.ssid));
    if (bssid_text) {
        if (parse_bssid(bssid_text, args.bssid) != 0) {
            fprintf(stderr, "rtw88ctl: invalid BSSID '%s'\n", bssid_text);
            return 1;
        }
        args.use_bssid = 1;
    }

    kern_return_t kr = IOConnectCallStructMethod(conn, kRTW88SyntheticAssociate,
                                                  &args, sizeof(args),
                                                  NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: synthetic-assoc failed: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Synthetic WPA2 SET20 dispatched for SSID '%s'%s%s.\n",
           ssid, bssid_text ? " BSSID " : "", bssid_text ? bssid_text : "");
    printf("No password/PMK was supplied; the common SET20 body should select Apple-RSN fallback.\n");
    return 0;
}
...
static int cmd_skywalk_prepare(io_connect_t conn)
{
    printf("Triggering one-shot Skywalk prepareBSDInterface probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkPrepareProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk prepare probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk prepare probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_defer(io_connect_t conn)
{
    printf("Triggering one-shot Skywalk deferBSDAttach(true) probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkDeferProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk defer probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk defer probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_start(io_connect_t conn)
{
    printf("Triggering one-shot Skywalk start(controller) probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkStartProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk start probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk start probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_enable(io_connect_t conn)
{
    printf("Triggering one-shot non-BSD IO80211 system-enable latch probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkEnableProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk system-enable latch probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk system-enable latch probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_userpower(io_connect_t conn)
{
    printf("Triggering one-shot Skywalk setPoweredOnByUser(true) probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkUserPowerProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk user-power probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk user-power probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_powernotify(io_connect_t conn)
{
    printf("Triggering one-shot Skywalk POWER_CHANGED notification probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkPowerNotifyProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk power-notify probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk power-notify probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_bsdlink(io_connect_t conn)
{
    printf("Triggering one-shot BSD companion IFF_UP|IFF_RUNNING probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkBSDLinkProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk BSD-link probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk BSD-link probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_running(io_connect_t conn)
{
    printf("Triggering one-shot private Skywalk setRunningState(true) probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkRunningProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk running-state probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk running-state probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_privatelink(io_connect_t conn)
{
    printf("Triggering one-shot private Skywalk setLinkState(Up) probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkPrivateLinkProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk private-link probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk private-link probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_reportlink(io_connect_t conn)
{
    printf("Triggering one-shot Skywalk reportLinkStatus(Active, IEEE80211) probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkReportLinkProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: Skywalk report-link probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Skywalk report-link probe returned successfully.\n");
    return 0;
}

static int cmd_skywalk_controllerpower(io_connect_t conn)
{
    printf("Triggering one-shot controller changePowerStateToPriv(2) probe...\n");
    fflush(stdout);
    kern_return_t kr = IOConnectCallScalarMethod(conn,
                                                  kRTW88SkywalkControllerPowerProbe,
                                                  NULL, 0, NULL, NULL);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "rtw88ctl: controller power-desire probe returned: %s (0x%x)\n",
                mach_error_string(kr), kr);
        return 1;
    }
    printf("Controller power-desire probe returned successfully.\n");
    return 0;
}
...
static void usage(const char *argv0)
{
    fprintf(stderr,
        ...
        "  synthetic-assoc <ssid> [bssid]\n"
        "                           Inject one credential-less WPA2 SET20 diagnostic\n"
        ...
        "  skywalk-prepare          Manually run one prepareBSDInterface probe\n"
        "  skywalk-defer            Manually run one deferBSDAttach(true) probe\n"
        "  skywalk-start            Manually run one Skywalk start(controller) probe\n"
        "  skywalk-enable           Manually run one controller Skywalk enable probe\n"
        "  skywalk-userpower        Manually run one setPoweredOnByUser(true) probe\n"
        "  skywalk-powernotify      Manually post one Skywalk POWER_CHANGED notification\n"
        "  skywalk-bsdlink          Manually set BSD companion IFF_UP|IFF_RUNNING only\n"
        "  skywalk-running          Manually set private Skywalk running state only\n"
        "  skywalk-privatelink      Manually set private Skywalk link state Up only\n"
        "  skywalk-reportlink       Manually report Skywalk link status Active only\n"
        "  skywalk-controllerpower  Manually request controller device power desire 2 only\n"
        "\n"
        "Examples:\n"
        "  %s scan -w 10            Scan for 10 seconds\n"
        "  %s list                  Show found networks\n"
        "  %s connect \"MyWiFi\" \"password123\"\n"
        "  %s synthetic-assoc \"MyWiFi\" aa:bb:cc:dd:ee:ff\n"
        "  %s status\n"
        "\n"
        "Notes:\n"
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(cmd, "scan") == 0) {
        ...
        if (argc < 3) {
            ...
            if (!password && isatty(STDIN_FILENO)) {
                password = getpass("WPA/WPA2 password (leave empty for open): ");
                if (!password) {
                    fprintf(stderr, "rtw88ctl: unable to read password\n");
                    ret = 1;
                }
            }
            if (ret == 0)
                ret = cmd_connect(conn, argv[2], password);
        }

    } else if (strcmp(cmd, "synthetic-assoc") == 0) {
        if (argc < 3) {
            fprintf(stderr, "rtw88ctl: synthetic-assoc requires SSID [BSSID]\n");
            ret = 1;
        } else {
            ret = cmd_synthetic_associate(conn, argv[2], argc >= 4 ? argv[3] : NULL);
        }
    ...
    } else if (strcmp(cmd, "skywalk-prepare") == 0) {
        ret = cmd_skywalk_prepare(conn);

    } else if (strcmp(cmd, "skywalk-defer") == 0) {
        ret = cmd_skywalk_defer(conn);

    } else if (strcmp(cmd, "skywalk-start") == 0) {
        ret = cmd_skywalk_start(conn);

    } else if (strcmp(cmd, "skywalk-enable") == 0) {
        ret = cmd_skywalk_enable(conn);

    } else if (strcmp(cmd, "skywalk-userpower") == 0) {
        ret = cmd_skywalk_userpower(conn);

    } else if (strcmp(cmd, "skywalk-powernotify") == 0) {
        ret = cmd_skywalk_powernotify(conn);

    } else if (strcmp(cmd, "skywalk-bsdlink") == 0) {
        ret = cmd_skywalk_bsdlink(conn);

    } else if (strcmp(cmd, "skywalk-running") == 0) {
        ret = cmd_skywalk_running(conn);

    } else if (strcmp(cmd, "skywalk-privatelink") == 0) {
        ret = cmd_skywalk_privatelink(conn);

    } else if (strcmp(cmd, "skywalk-reportlink") == 0) {
        ret = cmd_skywalk_reportlink(conn);

    } else if (strcmp(cmd, "skywalk-controllerpower") == 0) {
        ret = cmd_skywalk_controllerpower(conn);
